...
class Cosmos(object):
    ...
    def __init__(self, opts={}):
        ...
        key = opts['key']
        if url == None:
            url = os.environ[Constants.AZURE_COSMOSDB_NOSQL_URI]
        if key == None:
            key = os.environ[Constants.AZURE_COSMOSDB_NOSQL_RW_KEY1]
    ...
    def last_request_charge(self):
        if Constants.REQUEST_CHARGE_HEADER in self._record_diagnostics.headers:
            return self._record_diagnostics.headers[Constants.REQUEST_CHARGE_HEADER]
        else:
            return -1

    def last_activity_id(self):
        if Constants.ACTIVITY_ID_HEADER in self._record_diagnostics.headers:
            return self._record_diagnostics.headers[Constants.ACTIVITY_ID_HEADER]
        else:
            return None
