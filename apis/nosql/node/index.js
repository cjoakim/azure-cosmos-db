...
const querySpec = {
    query: "select * from products p where p.pk=@pk",
    parameters: [
        {
            name: "@pk",
            value: items[2].pk
...
// Delete item
const { statusCode } = await container.item(items[2].id, items[2].pk).delete();
